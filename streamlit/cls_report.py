...
if len(seg) == 0:
    ...
    chart = alt.Chart(seg).mark_bar(color="#ff4b4b").encode(
        x=alt.X("TimeSec:Q", title="Time",
                 axis=alt.Axis(
                     labelAngle=-45,
                     labelExpr=f"floor(datum.value / 3600) + ':' + floor((datum.value % 3600) / 60) + ':' + datum.value % 60"
                 ),
                 scale=alt.Scale(domain=[seg["TimeSec"].min(), seg["TimeSec"].max()])),
